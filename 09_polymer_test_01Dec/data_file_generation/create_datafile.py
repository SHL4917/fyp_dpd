...
box_size = 15
polymer_conc = 0.1
polymer_structure = np.array([2, 3])
...
polymer_beadcount = np.round((particle_count * 0.1)).astype(int)
...
#%% Get coordinates of the polymer locations
...
#%% Write data to file
with open("data.txt", mode = "w") as line:
    line.write("LAMMPS " + description + "\n \n")
    
    line.write(str(particle_count) + " atoms\n")
    line.write(str(506 * (polymer_length - 1)) + " bonds\n")
    ...
    line.write(str(atom_types + 1) + " atom types\n")
    ...
    line.write("Masses\n \n")
    line.write("1 " + str(mass_solvent) + "\n")
    line.write("2 " + str(mass_peo) + "\n")
    line.write("3 " + str(mass_pee) + "\n")
    line.write("4 1\n")
    line.write("\n")
