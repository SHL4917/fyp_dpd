...
#%% Load polymer numbers
polymer_num = np.loadtxt("e4o4_num.txt").astype("int")
...
#%% Unwrap polymers - Ensure all polymers are not split due to PBC
for i in range(np.shape(polymer_coord)[3]):
    polymer_coord[:, :, :, i] = unwrap_polymers(polymer_coord[:, :, :, i], 
                                                boxlengths)
    print(i)

#%% Save the polymer_coordinate
np.save("polymer_coords.npy", polymer_coord)
