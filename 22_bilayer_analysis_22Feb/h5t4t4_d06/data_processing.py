...
for i in range(np.shape(coord1)[2]):
    ...
    t1, t2, t3, axis = get_bilayer_histogram(c1[:, 0:3], c2[:, 0:3], 
                                                       c3[:, 0:3], 1000, 
                                                       plane, bound)
    if i == 0:
        hist1 = t1.copy()
...
for i in range(np.shape(coord1)[2]):
    ...
    t_profile, axis = get_p_profile(np.concatenate([c1, c2, c3]), num_bins, 
                                    plane, bound)
...
plt.figtext(.5,.91,
            "Averaged over 8 Timesteps", fontsize=11, ha='center')
