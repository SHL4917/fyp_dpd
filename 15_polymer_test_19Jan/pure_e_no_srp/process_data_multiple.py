...
r_g_array = np.zeros(13)
mass_array = np.zeros(13)
...
for i in range(5, 18, 1):
    ...
    r_g_array[i - 5] = np.average(r_g_ave)
    mass_array[i - 5] = i * 396
...
plt.figtext(.5,.90,
            "For a bond length of $0.5R_c$ with no SRP", 
            fontsize=12, ha='center')
ax.set_ylabel("$ln(R_g)$")
ax.set_xlabel("$ln(M)$")
plt.legend(loc = "lower right")

# fig.savefig('bondlength_0.5_no_srp.png', format='png', dpi=1200, bbox_inches='tight')
