...
r_g_array = np.zeros(11)
mass_array = np.zeros(11)
...
for i in range(4, 15, 1):
    ...
    r_g_array[i - 4] = np.average(r_g_ave)
    mass_array[i - 4] = i * 396
...
plt.figtext(.5,.90,
            "For a bond length of $0.5R_c$", 
            fontsize=12, ha='center')
ax.set_ylabel("$ln(R_g)$")
ax.set_xlabel("$ln(M)$")
plt.legend(loc = "lower right")

# fig.savefig('bondlength_0.5.png', format='png', dpi=1200, bbox_inches='tight')
