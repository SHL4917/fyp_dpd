...
#%% Plot the a_ij = 25 viscosity curve
data = np.genfromtxt("temp_viscosity_data.txt")

fig, ax = plt.subplots()
ax.plot(data[:, 0], data[:, 2])

ax.scatter(t_simulation[25], viscosity[25])
...
ax.set_ylabel("$\eta\ (Pa\ s)$", fontsize = 12)
fig.legend(handles = (ax.get_children() ), 
           labels = (["aij = 25"] + ["Viscosity Curve"]), loc = "center right")

plt.show()

#%% Plot the velocity profile for a_ij = 25
fig, ax = plt.subplots()
x = vprofile_vx[25][45, :, 1]
y = vprofile_vx[25][45, :, 3]
...
plt.figtext(.5,.97, "$x$ Velocity Profile in the $z$ direction", 
            fontsize=15, ha='center')
plt.figtext(.5,.91, "For $a_{ij}=25$ and $\gamma^* = 0.15$, step 45,000", 
            fontsize=12, ha='center')
ax.set_xlabel("$z\ (r^*)$", fontsize = 12)
ax.set_ylabel("$V_x^*$", fontsize = 12)
ax.grid(b = True, which = "major", linestyle = "--", lw = 1)

ax.set_xlim([0, 10])

plt.show()
fig.savefig('velo_profile_aij_25.png', format='png', dpi=1200, 
            bbox_inches='tight')
