...
rc = [float(x.split("_")[-1]) for x in subfolders]

rc_values = {}
for i in range(len(rc)):
    rc_values[rc[i]] = subfolders[i]
...
for key, entry in rc_values.items():
...
for key, entry in rc_values.items():
    ...
    ax.scatter(x, y, s = 15, label = "$r_c^* = " + str(key) + "$")
...
fig.savefig("velo_profile_rc_vary.png", format='png', dpi=1200, bbox_inches='tight')  
...
for key, entry in rc_values.items():
    ...
    print(y_ave, viscosity_star[key])
...
plt.title("Bulk Stress $\sigma_{xz}$ against Cut-off Distance", fontsize = 14)
ax.scatter(rc_val, stress_xz, s = 20)
ax.plot(rc_val, stress_xz, ls = "--", linewidth = 1)
ax.set_xlabel("$r_c^*$", fontsize = 12)
ax.set_ylabel("$\sigma_{xz}^*$", fontsize = 12)

ax.set_xlim([0.5, 3]) 
ax.set_ylim([0, 3])
plt.grid(b = True, which = "major", linestyle = "--", lw = 1)
fig.savefig("stress_xz_rc_vary.png", format='png', dpi=1200, bbox_inches='tight')
...
fig.savefig("viscosity_rc_vary.png", format='png', dpi=1200, bbox_inches='tight')  
